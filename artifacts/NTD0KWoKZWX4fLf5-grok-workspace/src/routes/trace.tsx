...
import { fetchLiveHops } from "@/lib/garuda/live";
...
function TracePage() {
  ...
  const [source, setSource] = useState<"sim" | "eth" | "btc">("sim");
  ...
  async function execute(w = wallet, cid = caseId, src = source) {
    ...
    if (src !== "sim") {
      const live = await fetchLiveHops({
        data: { address: w, chain: src === "btc" ? "bitcoin" : "ethereum" },
      });
      if (live.status === "SUCCESS" && live.hops.length) {
        extra = { [w]: live.hops };
        mode = "live";
        toast(`Live ledger: ${live.hops.length} outbound hops`);
      } else {
        toast(live.status === "RATE_LIMITED" ? "Live API rate-limited — using simulated ledger" : "Live lookup empty — using simulated ledger");
      }
    }
