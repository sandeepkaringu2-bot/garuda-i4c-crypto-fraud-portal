...
function scoreWallet(id: string, hop: number): number {
  const v = KNOWN_VASPS[id];
  const s = SANCTIONS[id];
  if (s) return 100;
  if (v?.kind === "mixer") return 100;
  if (v?.kind === "bridge") return 72;
  if (v?.jurisdiction === "India") return Math.max(8, 28 - hop * 2);
  if (v) return Math.max(12, 40 - hop * 2);
  return Math.max(10, 65 - hop * 5);
}
...
function analyzeTypology(edges: GraphEdge[]): TypologyHit[] {
  ...
  if (edges.some((e) => e.to in SANCTIONS || KNOWN_VASPS[e.to]?.kind === "mixer")) {
    const node = edges.find((e) => SANCTIONS[e.to] || KNOWN_VASPS[e.to]?.kind === "mixer")!.to;
    hits.push({
      node,
...
export function runTrace(
  ...
  const walk = (wallet: string, hop: number) => {
    ...
    } else if (!threat && !(vasp && vasp.kind === "vasp")) {
      endpoints.push({
        wallet,
        entity: "Unknown non-custodial wallet",
        kind: "unknown",
        risk: scoreWallet(wallet, hop),
        hops: hop,
        chainHint: "No further public hops in the indexed ledger",
        recoverable: false,
        amountUsdt: inboundAmount(edges, wallet),
      });
    }
  ...
  // Fill inbound amounts now that the graph is complete
